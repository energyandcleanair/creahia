...
# calpuff_files %>% make_tifs(grids = grids) # CHECK : not sure it is useful
...
#adm %<>% subset(country_id=='KHM') %>% head(10)
...
saveRDS(conc_adm, file.path(project_dir, paste0("conc_adms","_",scenario_prefix,".RDS")))

# ******************************************************************************
# If you start from here
# conc_adm <- readRDS(file.path(project_dir, "conc_adm.RDS"))
# scenario <- names(conc_adm)
...
paf <- conc_adm %>% compute_hia_paf(calc_causes=calc_causes, gemm=gemm, gbd=gbd, ihme=ihme)  #, conc_map=conc_adm)  # Added: conc_adm
# saveRDS(paf, file.path(project_dir, 'paf.RDS'))
saveRDS(paf, file.path(project_dir, paste0('paf',"_",scenario_prefix,'.RDS')))
