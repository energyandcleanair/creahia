...
hias <- lapply(creahia_versions, function(creahia_version){
  hia <- lapply(years, function(year){
    get_hia_deaths_pm_memo(year=year, res, grid, adm, adm_res, suffix, creahia_version)
  }) %>% bind_rows()
  saveRDS(hia, glue("validation/hias_{creahia_version}.RDS"))
  return(hia)
})
