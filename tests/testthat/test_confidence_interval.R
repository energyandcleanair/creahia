...
test_that("Order of estimates is consistent", {
  ...
  hia <- generate_uniform_exposure_hia(60, 10, calc_causes='GEMM and GBD')
  ...
  for(rr_sources in rr_sourcess) {
    hia <- generate_uniform_exposure_hia(60, 10, rr_sources=rr_sources)
...
test_that("Estimates are of the same sign with uniform exposure", {
  ...
  hia <- generate_uniform_exposure_hia(6, 5, rr_sources=RR_FUSION)
