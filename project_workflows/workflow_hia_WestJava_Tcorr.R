...
#make tifs
scenarios_to_process=c('bmas', 'bhas', 'coas', 'batas')
scenarios_to_process=c('batas')
calpuff_files %>% filter(scenario %in% scenarios_to_process, period=='annual',
                         speciesName %in% pollutants_to_process) %>%
  as.data.frame() %>% # for some reason, this fucntion was returning tibble
