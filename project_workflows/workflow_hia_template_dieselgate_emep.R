...
hia_fut <- hia_cost %>% creahia::get_econ_forecast(years = targetyears, pop_targetyr = 2024, GDP_scaling = T)
...
temp_name <- paste0("hia_fut_totals_",regs,"_",targetyears)
...
# for greater london
  ...
  hia_totals_x <- hia_totals%>%filter(iso3 == 'GBR')%>%filter(NAME_2=='Greater London')
  #hia_totals_x <- hia_totals_paris
  ...
  for (targetyears in seq(2009,2040)){
    ...
    temp_name <- paste0("hia_fut_totals_city_greater_london_",targetyears)
    assign(temp_name,hia_fut_totals)
  ...
  for (targetyears in seq(2009,2040)){
    ...
    hia_fut_totals <- get(paste0("hia_fut_totals_city_greater_london_",targetyears))%>%
      #  hia_fut_totals <- get(paste0("hia_fut_totals_",targetyears))%>%
      mutate(number = ifelse(Pollutant == "O3_8h",number * 1.96, number),
             cost_mn_currentUSD = ifelse(Pollutant == "O3_8h",cost_mn_currentUSD * 1.96, cost_mn_currentUSD),
             cost_bn_currentLCU = ifelse(Pollutant == "O3_8h",cost_bn_currentLCU * 1.96, cost_bn_currentLCU))
    ...
    hia_fut_totals_out<-hia_fut_totals_out %>% filter(!double_counted) %>%
      ...
      write_csv(file.path(output_dir,"temp",paste0('hia_results_greater_london_',targetyears,'.csv')))
  ...
  hia_totals_x <- hia_totals%>%filter(iso3 == 'FRA')%>%filter(NAME_1=='ÃŽle-de-France')
  #hia_totals_x <- hia_totals_paris
  ...
  for (targetyears in seq(2009,2040)){
    ...
    hia_fut_totals_out<-hia_fut_totals_out %>% filter(!double_counted) %>%
      ...
      write_csv(file.path(output_dir,"temp",paste0('hia_results_ÃŽle-de-France_',targetyears,'.csv')))
...
merged_df <- bind_rows(list_of_dfs)%>%
  write_csv(file.path(output_dir,paste0('hia_results_all_mod3.csv')))
...
csv_files <- list.files(file.path(output_dir,"temp"), pattern = "hia_results_greater_london", full.names = TRUE)
list_of_dfs <- lapply(csv_files, read.csv)
#print(list_of_dfs)
merged_df <- bind_rows(list_of_dfs)%>%
  write_csv(file.path(output_dir,paste0('hia_results_city_sus_greater_london.csv')))
...
#print(list_of_dfs)
merged_df <- bind_rows(list_of_dfs)%>%
  write_csv(file.path(output_dir,paste0('hia_results_city_sus_ÃŽle-de-France.csv')))
...
emis_inv <- emis_inv%>%
  ...
  df <- read.csv(file.path(output_dir,paste0('hia_results_city_sus_ÃŽle-de-France.csv')))
  for (targetyears in seq(2009,2040)){
    for (scale_scenario in scenarios_to_process){
      sf_a <-emis_inv %>%
        filter(cal_year == targetyears) %>%
  ...
  write.csv(df,file.path(output_dir,paste0('hia_results_city_real_ÃŽle-de-France.csv')))
  ...
  df <- read.csv(file.path(output_dir,paste0('hia_results_city_sus_greater_london.csv')))
  for (targetyears in seq(2009,2040)){
    for (scale_scenario in scenarios_to_process){
      sf_a <-emis_inv %>%
        filter(cal_year == targetyears) %>%
  ...
  write.csv(df,file.path(output_dir,paste0('hia_results_city_real_greater_london.csv')))
  
# for whole region  
  df <- read.csv(file.path(output_dir,paste0('hia_results_all_sus_mod3.csv')))
  for (targetyears in seq(2009,2040)){
    for (scale_scenario in scenarios_to_process){
      sf_a <-emis_inv %>%
        filter(cal_year == targetyears) %>%
  ...
  write.csv(df,file.path(output_dir,paste0('hia_results_all_real_mod3.csv')))
