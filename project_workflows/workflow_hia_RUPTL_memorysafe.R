...
output_dir <- file.path(project_dir, "RUPTL2025","HIA") ; if (!dir.exists(output_dir)) dir.create(output_dir) # Where to write all HIA files
...
grid_ll <- grid_raster %>% terra::project('epsg:4326', res=.01) %>% terra::extend(200)
conc_ll <- get_conc_baseline(species=unique(calpuff_files$species), grid_raster=grid_ll, no2_targetyear = 2023) # 2020 # Target year of model simulations (CALPUFF and WRF)
conc_base <- conc_ll
conc_base$conc_baseline %>% lapply(terra::project, y=grid_raster, method='average')

#conc_base <- get_conc_baseline(species=unique(calpuff_files$species), grid_raster=grid_raster, no2_targetyear = 2023) # 2020 # Target year of model simulations (CALPUFF and WRF)

conc_base %>% saveRDS(file.path(output_dir, 'conc_base.RDS'))
conc_base <- readRDS(file.path(project_dir, 'HIA', 'conc_base.RDS'))
...
for (scen in scenarios_to_process[queue]) {
  ...
  exposure_rasters$conc %>% rast %>%
    terra::extract(regions, sum, na.rm=T) %>%
...
hia <- wrappers.compute_hia_two_images(perturbation_rasters=concs$conc_perturbation,       # perturbation_rasters=raster::stack(perturbation_map)
                                       baseline_rasters=concs$conc_baseline,  # baseline_rasters=raster::stack(who_map)
                                       regions=regions,
                                       scenario_name='1ug',
...
saveRDS(hia, file.path(output_dir, paste0('hia_GEMM_1ug.RDS')))

hia <- readRDS(file.path(output_dir, paste0('hia_GEMM_1ug.RDS')))
...
hia_cost <- get_hia_cost(hia$hia, valuation_version="viscusi", current_year=2024)

targetyears = 2000:2070 #1984:2070 #emis$year %>% unique
hia_fut <- hia_cost %>% get_econ_forecast(forecast_years=targetyears, reference_year=2024)

#hia_fut_indo <- hia_fut %>% filter(iso3=='IDN')

output_dir_old <- file.path(project_dir, "HIA")

#get costs and deaths per t emissions
require(pbapply)

queue <- scenarios_to_process %>% paste0('hia_', .,'.csv') %>% file.path(output_dir, .) %>% file.exists() %>% not

scenarios_to_process[queue] %>%
  pblapply(function(run) {
    ...
    paste0('exp_',run,'.csv') %>% file.path(output_dir, .) %>%
      ...
      left_join(hia_fut %>%
                  select(region_id, iso3, pop, pollutant, outcome, cause, double_counted, year, estimate, number, cost_mn_currentUSD)) %>%
      mutate(across(c(number, cost_mn_currentUSD), ~.x * exposure/pop/modeled_emissions)) %>%
      group_by(cluster, year, outcome, cause, pollutant, emitted_species, double_counted, estimate) %>%
      summarise(across(c(number, cost_mn_currentUSD), sum, na.rm=T)) %>%
    write_csv(paste0('hia_',run,'.csv') %>% file.path(output_dir, .))
  })

scenarios_to_process %>% paste0('hia_',.,'.csv') %>% file.path(output_dir, .) %>%
  pblapply(read_csv) %>% bind_rows() -> hia_per_t

hia_per_t %>% saveRDS(file.path(output_dir, 'hia_per_t.RDS'))

#HIA for scenarios
#depends on creapuff/project_workflows/Indonesia_RUPTL2025/indonesia_RUPTL_emissions_processing.R
#indonesia_RUPTL_read_plantdata.R
source('~/CALPUFF/creapuff/project_workflows/Indonesia_RUPTL2025/read_RUPTL2025_emissions.R') #needs to be updated

hia_per_t <- readRDS(file.path(output_dir, 'hia_per_t.RDS'))
emis <- readRDS(file.path(emissions_dir, 'indonesia_RUPTL_emission_pathways_July_2026.RDS'))

emis %>% ungroup %>%
  filter(COD<=year, year_retire>=year,
         Status=='operating' | year>2025) %>%
  rename(emitted_species=pollutant) %>%
  group_by(cluster, year) %>%
  mutate(percent_done=scales::percent(cur_group_id()/n_groups(.), accuracy=.01)) %>%
  group_by(percent_done, .add=T) %>%
  group_modify(function(df, group) {
    message('cluster: ', group$cluster, ' year: ', group$year, '; completed: ', group$percent_done)
    df %>%
      inner_join(hia_per_t %>% filter(cluster==group$cluster, year==group$year)) %>%
      group_by(CFPP.name, Owner, province, region, Latitude, Longitude, scenario, outcome, cause, pollutant, double_counted, estimate) %>%
      mutate(across(c(number, cost_mn_currentUSD), ~.x*emissions_t)) %>%
      summarise(across(c(number, cost_mn_currentUSD), sum),
                across(c(MW, GWh), unique))
  }) %>% select(-percent_done) %>%
  mutate(across(c(number_per_TWh=number, cost_mn_currentUSD_per_TWh=cost_mn_currentUSD), ~.x/(GWh/1000))) ->
  hia_scenarios
...
hia_scenarios %>%
  group_by(scenario, plant_category=ifelse(Owner=='captive', 'captive', 'utility'),
           outcome, cause, pollutant, double_counted, year, estimate) %>%
  summarise(across(c(number, cost_mn_currentUSD), sum)) ->
...
hia_scenarios_totals %<>% filter(!double_counted, !grepl("economic costs", outcome)) %>%
  group_by(scenario, plant_category, year, estimate) %>%
  summarise(across(c(number=cost_mn_currentUSD), sum)) %>%
  mutate(outcome = "economic costs", unit="million USD", pollutant='All', double_counted = F) %>%
...
hia_scenarios_totals %<>% filter(!double_counted, grepl("Death", outcome)) %>%
  ...
  mutate(outcome = "deaths, total", cause='AllCause', pollutant='All', double_counted = T) %>%
  bind_rows(hia_scenarios_totals %>% filter(outcome != "deaths, total"))
...
hia_scenarios_totals %>% ungroup %>%
  ...
  theme_crea_new() + scale_color_crea_d('dramatic', guide=guide_legend(nrow = 1)) +
  theme(legend.position = 'top', axis.text.x=element_text(vjust=-.2)) +
  labs(title='Air pollution-related deaths by scenario', y='cases per year', x='') +
  x_at_zero(labels=scales::comma) + snug_x
quicksave(file.path(output_dir, 'Air pollution-related deaths by scenario.png'), scale=1)

hia_scenarios_totals %>% ungroup %>%
  filter(estimate=='central', outcome=='economic costs', year>=2010) %>%
  group_by(scenario, year, estimate) %>% summarise(across(number, sum, na.rm=T)) %>%
  ungroup %>%
  complete(scenario, year=2000:2056, estimate) %>% replace_na(list(number=0)) %>%
  filter(year>=2010) %>%
...
hia_scenarios_totals %>%
  add_long_names() %>%
  select(scenario, plant_category, outcome=outcome_long, cause=cause_long, pollutant, double_counted, estimate, number, year) %>%
  pivot_wider(names_from=estimate, values_from=number) %>%
  relocate(high, .after = low) %>%
  write_csv(file.path(output_dir, 'annual HIA.csv'))


hia_scenarios_totals %>% filter(year>2024) %>%
  group_by(scenario, plant_category, estimate, outcome, cause, pollutant, double_counted) %>%
  summarise(across(c(number, cost_mn_currentUSD), sum)) ->
  hia_cum

hia_cum %>% filter(estimate=='central', !double_counted, grepl('Death', outcome)) %>%
  group_by(scenario, plant_category, estimate) %>% summarise(across(number, sum)) %>%
  ungroup %>%
  mutate(scenario = fct_reorder(scenario, number, .fun = sum, .desc = FALSE)) %>%
  ggplot(aes(scenario, number, fill=scenario, alpha=plant_category)) + geom_col() +
  theme_crea_new() +
  scale_fill_crea_d('dramatic', guide='none') +
  scale_alpha_manual(values=c(.75,1)) +
  x_at_zero(labels=scales::comma) +
  coord_flip() +
  labs(title='Cumulative air pollution deaths by scenario',
       subtitle='2025 to end-of-life',
       x='', y='', alpha='plant category')
quicksave(file.path(output_dir, 'Cumulative deaths by scenario.png'), scale=1)

hia_cum %>%
  filter(estimate=='central', outcome=='economic costs', !double_counted) %>%
  group_by(scenario, plant_category, estimate) %>% summarise(across(number, sum, na.rm=T)) %>%
  ungroup %>%
  mutate(scenario = fct_reorder(scenario, number, .fun = sum, .desc = FALSE)) %>%
  ggplot(aes(scenario, number/1e3, fill=scenario, alpha=plant_category)) + geom_col() +
  theme_crea_new() +
  scale_fill_crea_d('dramatic', guide='none') +
  scale_alpha_manual(values=c(.75,1)) +
  x_at_zero(labels=scales::comma) +
  coord_flip() +
  labs(title='Cumulative air pollution costs by scenario',
       subtitle='2025 to end-of-life',
       x='', y='USD bn', alpha='plant category')
quicksave(file.path(output_dir, 'Cumulative costs by scenario.png'), scale=1)


hia_cum %>%
  add_long_names() %>%
  select(scenario, plant_category, outcome=outcome_long, cause=cause_long, pollutant, double_counted,
         estimate, number) %>%
  pivot_wider(names_from=estimate, values_from=number) %>%
  relocate(high, .after = low) %>%
