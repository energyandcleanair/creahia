...
# remotes::install_github("energyandcleanair/creahia@master") # Before the fix
...
get_hia_deaths_pm <- function(year, res, grid, adm, adm_res, suffix){
...
hias <- lapply(c(2015), function(year){
  get_hia_deaths_pm(year, res, grid, adm, adm_res, suffix)
})

memoise::has_cache(get_hia_deaths_pm_memo)(year, res, grid, adm, adm_res)


saveRDS(hias, glue("validation/hias_{version}.RDS"))

hias_files <- list.files("validation", "hias_.*.RDS", full.names = T)
