...
bbox <- NULL #creaexposure::get_bbox("IN")
res <- creaexposure::RES_15_MIN
...
get_hia_deaths_pm <- function(year, res, grid, adm, adm_res){

  print(year)

  # Get PM2.5 exposure map from van Donkelaar
  pm25 <- creaexposure::data.basemap_pm25(year=year, res=res, pop=grid, use_cache = T)
...
hias <- lapply(years, function(year){
  get_hia_deaths_pm_memo(year, res, grid, adm, adm_res)
})
