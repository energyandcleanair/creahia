# This is a comparison of creahia PM2.5 global mortality estimates from various models with the literature
...
# Burnett, Richard T., Joseph V. Spadaro, George R. Garcia, and C. Arden Pope.
# “Designing Health Impact Functions to Assess Marginal Changes in Outdoor Fine Particulate Matter.” Environmental Research 204 (March 1, 2022): 112245. https://doi.org/10.1016/j.envres.2021.112245.
# The global total number of deaths for a 100% exposure reduction using the Fusion model (8.2 million; 95% confidence interval: 5.9–10.0)
# Year 2019
...
validation <- list(
  tibble(
    region_id="Global",
    source="Burnett et al. 2022",
    rr_source=creahia::RR_FUSION,
    deaths=c(8.2e6),
    year=c(2019),
    pm25=c(NA)
  )
...
creahia_versions <- packageVersion("creahia")
suffix <- ""


get_hia_deaths_pm <- function(year, res, grid, adm, adm_res, suffix, creahia_version, rr_source){
  ...
  # Computing HIA
  if(creahia_version %in% c("0.3.0", "0.3.1")){
...
lapply(creahia_versions, function(creahia_version){
  mapply(function(year, res, grid, adm, adm_res, suffix, creahia_version, rr_source){
    tryCatch({
      hia <- get_hia_deaths_pm_memo(year, res, grid, adm, adm_res, suffix, creahia_version, rr_source)
  ...
  adm_res=adm_res,
  suffix=suffix,
  creahia_version=creahia_version,
  rr_source=validation$rr_source,
  SIMPLIFY=F)
})
