...
wb_ind = read_csv('World Bank indicators.csv')

pop <- read_csv('IHME_GBD_2019_POP_2019_Y2020M10D15.CSV') %>%  #read.csv('2017 data/IHME_GBD_2017_POP_2015_2017.CSV') %>%
  filter(year_id==2019, tolower(sex_name)=='both') %>% rename(country = location_name)
read_xlsx('IHME_GBD_2019_GBD_LOCATION_HIERARCHY_Y2020M10D15.XLSX', .name_repair = make.names) %>% #read_xlsx('2017 data/IHME_GBD_2017_ALL_LOCATIONS_HIERARCHIES.XLSX') %>%
  sel(location_id=matches('location.id', ignore.case = T),
      level=matches('^level$', ignore.case = T)) %>%
  right_join(pop) %>% filter(level==3) %>% distinct() -> pop
pop.total <- pop %>% filter(age_group_name == 'All Ages')
asthma.new <- read.csv('new asthma cases.csv')
asthma.erv <- read_xlsx('Anenberg EHP 2018 results.xlsx', sheet='pm totconc') %>%
  filter(!is.na(ID)) %>%
  rename(country=Country, exac.0to17=exac_0_17_base, exac.18to99=exac_18_99_base,
         exac.0to99=exac_0_99_base)
PTB <- read_xlsx('2014 National preterm birth rates.xlsx', skip=3, col_names = F, n_max=300)
nms <- read_xlsx('2014 National preterm birth rates.xlsx', skip=1, col_names = F, n_max=2)
nms[2, ] <- ifelse(is.na(nms[2, ]), nms[1, ], nms[2, ])
names(PTB) <- unlist(nms[2, ]) %>% make.names(unique=T)
PTB %<>% filter(!is.na(PTB.rate)) %>% rename(country=Country)
...
death.all.cause <- read_csv('IHME-GBD_2019_DATA-death_yll_yld.csv') %>%  #read.csv('2017 data/IHME-GBD_2017_DATA.csv') %>%
...
deaths.crude <- read_csv('IHME-GBD_2019_DATA-death_yll_yld.csv') %>%  #read.csv('2017 data/IHME-GBD_2017_DeathRates.csv') %>%
...
death.child.lri <- read_csv('IHME-GBD_2019_DATA-death_yll_yld.csv') %>%  #read.csv('IHME-GBD_2017_DATA.csv') %>%
...
yld <- read_csv('IHME-GBD_2019_DATA-death_yll_yld.csv') %>%  #read.csv('2017 data/IHME-GBD_2017_DATA.csv') %>%
...
death.totcp <- read_csv('IHME-GBD_2019_DATA-TotCV,TotResp.csv') %>%  #read.csv('2017 data/IHME-GBD_2017_DATA.csv') %>%
...
asthma.prev <- read_csv('IHME-GBD_2019_DATA-asthma.csv') %>%  #read.csv('2017 data/IHME-GBD_2017_Asthma.csv') %>%
...
asthma.prev <- read_csv('IHME-GBD_2019_DATA-asthma.csv') %>% #read.csv('IHME-GBD_2017_Asthma.csv') %>%
  filter(measure_name %in% c('Incidence', 'Prevalence'),
         metric_name == 'Number') %>%
  gather_ihme %>% filter(age_name != 'All Ages') %>%
  group_by(measure_name, country, location_id, estimate) %>%
  ...
  ihme_getrate() %>% bind_rows(asthma.prev)

read_csv('IHME-GBD_2019_DATA-asthma.csv') %>% #read.csv('IHME-GBD_2017_Asthma_AllAges.csv') %>%
